...
Consider deploying an app: you need to build Docker images AND run tests. These are independent—why
...
**Chain total:** 105s (sequential) **DAG total:** 75s (Build + Tests in parallel)

### 2. Replanning

Mid-execution changes are inevitable. Something fails. New information arrives. The agent needs to
adapt.
...
This enables **partial success**: maybe you can't deploy to prod, but staging works. Maybe
notifications fail, but the core operation completes.
