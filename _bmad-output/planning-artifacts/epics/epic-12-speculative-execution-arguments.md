...
> **⚠️ AUDIT TECHNIQUE (2026-01-26): Code Speculation Existant = STUB**
>
> Le code dans `src/dag/speculation/` (Story 3.5-1/3.5-2) est **obsolète et incomplet**:
>
> | Fichier | Status | Action |
> |---------|--------|--------|
> | `speculation-manager.ts` | ✅ OK | Réutiliser (threshold, metrics, feedback) |
> | `speculative-executor.ts` | ❌ STUB | **Réécrire entièrement** |
> | `integration.ts` | 🟡 | Adapter pour nouveau executor |
>
> **Problèmes critiques de `SpeculativeExecutor`:**
> 1. Utilise `DenoSandboxExecutor` au lieu de `WorkerBridge` → pas de traçabilité
> 2. `generateSpeculationCode()` retourne un **placeholder** (pas d'exécution réelle!)
> 3. Pas d'intégration avec `argument-resolver.ts`
> 4. Pas de `canSpeculate()` security guard
>
> **Ce qui est réutilisable:**
> - `SpeculationManager`: logique threshold, metrics hit/miss, pattern reinforcement
> - `SpeculationConfig`, `SpeculationMetrics` types
> - Cache TTL logic (à extraire dans fichier séparé)
>
> **Impact sur estimations:** Stories 12.4-12.6 = réécriture, pas modification.
...
> **⚠️ RÉÉCRITURE REQUISE (2026-01-26)**
>
> L'actuel `src/dag/speculation/speculative-executor.ts` est un **STUB** qui ne fait pas
> d'exécution réelle. Il faut réécrire entièrement avec WorkerBridge.
> Voir audit technique en haut de ce document.
...
- `src/speculation/speculative-executor.ts` - **RÉÉCRITURE COMPLÈTE** avec WorkerBridge (~300 LOC)
  - Supprimer `DenoSandboxExecutor`, injecter `WorkerBridge`
  - Implémenter vraie exécution (pas placeholder)
  - Intégrer `argument-resolver.ts`
  - Intégrer `canSpeculate()` guard (Story 12.3)
...
**Files à garder (réutilisables):**

- `src/dag/speculation/speculation-manager.ts` - Threshold, metrics, feedback ✅

**Note:** Requires Story 10.5 "Architecture Unifiée" to be completed first (WorkerBridge integration
in ControlledExecutor).

**Estimation:** 4 jours (réécriture, pas modification)
...
## Epic 12 Summary (Phase 1-2)

| Story | Titre                                        | FRs            | Estimation | Status |
| ----- | -------------------------------------------- | -------------- | ---------- | ------ |
| 12.1  | Context Initialization & Result Accumulation | FR1, FR2       | 1j         | 🟡 Partiel |
| 12.2  | Argument Resolver                            | FR3, FR6, FR9  | **0j**     | ✅ DONE |
| 12.3  | Security Guard (canSpeculate)                | FR5            | 1j         | 🔴 Todo |
| 12.4  | Real Speculative Execution                   | FR4, FR7, FR11 | **4j**     | ⚠️ RÉÉCRITURE |
| 12.5  | Speculation Cache & Validation               | FR10           | 1.5j       | 🟡 Partiel |
| 12.6  | Per-Layer Speculation                        | FR8            | 2j         | 🔴 Todo |
| 12.7  | Argument-Aware Learning                      | (ext FR2)      | 2-3j       | 🔴 Todo |

**Total Phase 1-2: ~12-13 jours** (ajusté après audit 2026-01-26)
...
### Story 12.9: Mock Registry & Exploration Learning

**Prerequisite:** Spike `2026-01-26-spike-mock-registry-exploration-learning.md` doit être complété

**As a** learning system, **I want** to implement mock registry and exploration learning based on
spike findings, **So that** SHGAT learns effectively from exploration traces and exploratory
dry-runs use high-quality, auto-curated mocks.
...
*(À définir après le spike - basé sur les décisions ADR)*
...
- **Spike:** `_bmad-output/planning-artifacts/spikes/2026-01-26-spike-mock-registry-exploration-learning.md`
...
*(À définir après le spike)*

**Estimation:** 3-4 jours (implémentation seule, après spike)
...
## Epic 12 Summary (Updated 2026-01-26)

| Story | Titre                                        | FRs                 | Phase       | Estimation | Note |
| ----- | -------------------------------------------- | ------------------- | ----------- | ---------- | ---- |
| 12.1  | Context Initialization & Result Accumulation | FR1, FR2            | Foundation  | 1j         | `sanitize-for-storage.ts` ✅ existe |
| 12.2  | Argument Resolver                            | FR3, FR6, FR9       | Foundation  | **0j**     | ✅ **DONE** (`argument-resolver.ts`) |
| 12.3  | Security Guard (canSpeculate)                | FR5                 | Foundation  | 1j         | |
| 12.4  | Real Speculative Execution                   | FR4, FR7, FR11      | Execution   | **4j**     | ⚠️ **RÉÉCRITURE** (stub actuel) |
| 12.5  | Speculation Cache & Validation               | FR10                | Execution   | 1.5j       | Extraire cache existant |
...
| SPIKE | **Mock Registry & Exploration Learning**     | -                   | Research    | 3-4j       | |
| 12.9  | **Mock Registry & Exploration Learning**     | FR14-15, FR17       | Implement   | 3-4j       | |
| 12.10 | **Exploration Results & LLM Selection**      | (ext FR12, FR16)    | Integration | 2-3j       | |

**Total Epic 12: ~25-28 jours** (12.2 déjà fait, mais 12.4 = réécriture)

> ⚠️ SPIKE doit être complété AVANT Story 12.9
...
DR-DSP (existing) ─────────────────────────────────▶│    Dry-Run      │
                                                    ...
                                                    ┌─────────────────────────┐
                                                    │ SPIKE: Mock Registry    │
Migration 024 (error_type) ────────────────────────▶│ & Exploration Learning  │
trace-feature-extractor.ts ────────────────────────▶│ (3-4j investigation)    │
                                                    └────────────┬────────────┘
                                                                 │
                                                                 ▼
                                                    ┌─────────────────────────┐
