...
// ============================================================================
// Real BGE-M3 Embedding Tests
// ============================================================================

import { EmbeddingModel } from "../../src/vector/embeddings.ts";

/**
 * Test SHGAT with real BGE-M3 embeddings
 *
 * This test downloads and uses the actual BGE-M3 model (~400MB first time).
 * Skip in CI with: SKIP_EMBEDDING_TESTS=1
 */
Deno.test({
  name: "SHGAT: trains with real BGE-M3 embeddings",
  ignore: Deno.env.get("SKIP_EMBEDDING_TESTS") === "1" || Deno.env.get("CI") === "true",
  async fn() {
    console.log("\n🔄 Loading BGE-M3 model (may take 60-90s first time)...");
    ...
    try {
      ...
      // Build tools with real embeddings
      const tools: Array<{ id: string; embedding: number[] }> = [];
      for (const tool of fixtureData.nodes.tools) {
        // Convert tool ID to readable description
      ...
      // Build capabilities with real embeddings
      const capabilities: Array<{
        id: string;
        embedding: number[];
        toolsUsed: string[];
      ...
      for (const cap of fixtureData.nodes.capabilities) {
        // Create descriptive text for capability
        const description = cap.id
          .replace("cap__", "")
          .replace(/_/g, " ");
        const embedding = await embedder.encode(description);
      ...
      console.log("🏋️ Training SHGAT with real embeddings...");
      ...
      console.log(`\n✅ Training complete: loss=${result.finalLoss.toFixed(4)}, accuracy=${result.finalAccuracy.toFixed(2)}`);

      // With real embeddings, should achieve better than random
      assertGreater(result.finalAccuracy, 0.3);

      // Test inference with semantic query
      console.log("\n🔍 Testing inference with semantic queries...");

      const testQueries = [
        "complete purchase for customer",
        "cancel an order",
      ...
      for (const query of testQueries) {
        ...
        for (const r of results.slice(0, 3)) {
          console.log(`    - ${r.capabilityId}: ${r.score.toFixed(4)}`);
        }
      }

      // Semantic test: "complete purchase" should rank checkout_flow high
      const checkoutQuery = await embedder.encode("complete purchase checkout");
      const checkoutResults = shgat.scoreAllCapabilities(checkoutQuery, []);

      const checkoutRank = checkoutResults.findIndex(r => r.capabilityId === "cap__checkout_flow");
      console.log(`\n  "complete purchase checkout" → cap__checkout_flow rank: ${checkoutRank + 1}`);

      // checkout_flow should be in top 2 for checkout-related query
      assertLess(checkoutRank, 2);
