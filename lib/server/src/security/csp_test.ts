...
Deno.test("buildCspHeader - custom script sources", () => {
  ...
  assertStringIncludes(csp, "script-src 'self' 'unsafe-inline' https://cdn.example.com");
});

Deno.test("buildCspHeader - custom connect sources", () => {
  const csp = buildCspHeader({ connectSources: ["wss://api.example.com", "https://api.example.com"] });
  assertStringIncludes(csp, "connect-src 'self' wss://api.example.com https://api.example.com");
});
...
Deno.test("injectCspMetaTag - injects after <head>", () => {
  ...
  assertStringIncludes(result, '<head><meta http-equiv="Content-Security-Policy"');
  assertStringIncludes(result, 'content="default-src \'none\'"');
  // Title should come after the meta tag
  assertEquals(result.indexOf("<meta"), result.indexOf("<title") - 70 < 0 ? result.indexOf("<meta") : result.indexOf("<meta"));
...
Deno.test("injectCspMetaTag - handles <head> with attributes", () => {
  ...
  const result = injectCspMetaTag(html, "default-src 'none'");
  assertStringIncludes(result, '<head lang="en"><meta http-equiv="Content-Security-Policy"');
});
...
Deno.test("injectCspMetaTag - escapes quotes in CSP value", () => {
  ...
  assertStringIncludes(result, `content="default-src 'none'; script-src 'self' 'unsafe-inline'"`);
});
