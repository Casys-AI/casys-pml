...
- **3 beam candidates** (deduplicated by target, best score per target,
  softmax-normalized to [0,1])
- **"none" option** — all candidates are wrong → all stored as negatives, no
  execution
...
Only positive examples count toward accuracy. Negatives contribute to
loss/gradients but not to the accuracy %.
