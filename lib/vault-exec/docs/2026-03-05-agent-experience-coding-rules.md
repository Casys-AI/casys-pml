...
Objectif : garder `vault-exec` lisible, stable et composable pour des agents (pas seulement pour des humains).

## 1) Architecture (non négociable)

- **Feature-sliced strict** :
  - `src/ingest/*` = parsing, decomposition, markdown/reporting d’ingestion.
  - `src/core/*` = primitives réellement transverses et stables (types, interfaces, utilitaires purs).
- **Pas de logique métier d’ingestion dans `core`** sans preuve de réutilisation multi-features.
...
- Ne pas perdre les wrappers (`cd`, `sleep`, `set -e`) : les garder comme **métadonnées de contexte**.
- La famille L2 principale doit refléter l’action métier (git/openclaw/python/http/etc.).
- Séparer :
  - `family` (dédup)
...
- Doc centrale (`docs/`) réservée aux plans d’implémentation, ADRs, architecture globale.
