...
export function resolveTemplate(
  ...
  if (!noteResults) {
    throw new Error(
      `Cannot resolve "{{${match[1]}}}": note "${ref.note}" has no results yet`,
    );
  }
  if (!(ref.output in noteResults)) {
    throw new Error(
      `Cannot resolve "{{${
        match[1]
      }}}": note "${ref.note}" has no output "${ref.output}"`,
    );
  }
...
export async function executeNode(
  ...
  if (node.type === "tool") {
    throw new Error(
      `Tool nodes not supported yet (Phase 2). Node: "${node.name}"`,
    );
  }

  throw new Error(
    `Unknown node type "${
      (node as CompiledNode).type
    }" for node "${node.name}"`,
  );
}
