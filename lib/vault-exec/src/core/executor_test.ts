...
import { executeGraph, executeNode, resolveTemplate } from "./executor.ts";
...
Deno.test("resolveTemplate - throws on missing note", () => {
  ...
  assertThrows(
    () => resolveTemplate("{{Missing.output}}", results),
    Error,
    "Missing",
  );
});
...
Deno.test("executeGraph - executes full DAG in order", async () => {
  ...
  const edges = new Map<string, string[]>([["A", []], ["B", ["A"]], ["C", [
    "B",
  ]]]);
...
Deno.test("executeGraph - supports machine mode with verbose=false", async () => {
  ...
  const edges = new Map<string, string[]>([["Input", []], ["Double", [
    "Input",
  ]]]);
  const graph: VaultGraph = { nodes, edges };
