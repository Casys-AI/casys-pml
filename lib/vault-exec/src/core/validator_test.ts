...
Deno.test("validate - node missing value/code/tool payload", () => {
  const notes = [
    makeNote("A", { outputs: ["output"] }, []),
  ];
  const graph = buildGraph(notes);
  const errors = validate(graph);
  assertEquals(errors.length, 1);
  assertEquals(errors[0].type, "unknown_type");
});

Deno.test("validate - node with ambiguous payload (value + code)", () => {
  const notes = [
    makeNote("A", {
      value: 1,
      code: "1 + 1",
