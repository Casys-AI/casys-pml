// gnn/backward_test.ts
import { assertEquals } from "jsr:@std/assert";
import { gnnTrainStep, serializeGnnParams, deserializeGnnParams } from "./backward.ts";
...
function makeNode(name: string, level: number, children: string[] = []): GNNNode {
  ...
  const emb = Array.from({ length: TEST_CONFIG.embDim }, (_, i) =>
    Math.sin(name.charCodeAt(0) * 7 + i * 0.3) * 0.5
  );
...
Deno.test("gnnTrainStep reduces MSE loss over 5 steps", () => {
  ...
  for (const node of nodes) {
    targets.set(
      node.name,
      Array.from({ length: TEST_CONFIG.embDim }, (_, i) =>
        Math.cos(node.name.charCodeAt(0) * 3 + i * 0.7) * 0.3
      ),
    );
  }
  ...
  assertEquals(
    losses[4] < losses[0],
    true,
    `Expected loss to decrease: first=${losses[0].toFixed(6)}, last=${losses[4].toFixed(6)}`,
  );
