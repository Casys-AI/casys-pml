...
import { edgeToVertex, vertexToEdge } from "./message-passing.ts";
...
Deno.test("vertexToEdge - no children returns copy of parent embedding", () => {
  ...
  assertEquals(result.length, 4);
  result.forEach((v, i) =>
    assertEquals(Math.abs(v - parent.embedding[i]) < 1e-10, true)
  );
});
...
Deno.test("edgeToVertex - no parents returns copy of child embedding", () => {
  ...
  assertEquals(result.length, 4);
  result.forEach((v, i) =>
    assertEquals(Math.abs(v - child.embedding[i]) < 1e-10, true)
  );
});
