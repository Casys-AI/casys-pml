 ...
 * - If FFI is unavailable (permissions/platform), it transparently falls back.
 * - Callers can keep pure-JS behavior by default.
 */
...
/** Force initialization and return whether BLAS acceleration is ready. */
export function initBlasAcceleration(): boolean {
  initBlas();
  return available;
}

/** Human-readable BLAS status for diagnostics/logging. */
export function getBlasStatus(): { available: boolean; path: string | null } {
  return { available, path: loadedPath };
}

/** Explicit teardown, primarily for tests and short-lived scripts. */
