...
function makeIntentCandidateGraph(): VaultGraph {
  const nodes = new Map<string, CompiledNode>([
    ...
    ["Load Deals", makeNode("Load Deals", { type: "value", value: [], outputs: ["deals"] })],
  ]);
...
Deno.test("candidate validation differentiates invalid vs valid payload per target", () => {
  ...
  const followUp = validateRuntimeInputsForGraph(followUpGraph, payloadForReport);
...
Deno.test("strict target schema behavior remains unchanged for extra fields", () => {
  ...
  assert(
    result.issues.some((issue) => issue.kind === "extra" && issue.path.endsWith("/unexpected")),
