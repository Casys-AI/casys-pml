...
const SYSTEM_PROMPT =
  `You are a router for a vault-exec system. Given a user intent and a list of available notes (computation nodes), identify which note best answers the intent.
...
function buildUserPrompt(intent: string, notes: VaultNote[]): string {
  const noteList = notes
    .map((n) => {
      const deps = n.wikilinks.length > 0
        ? ` (depends on: ${n.wikilinks.join(", ")})`
        : "";
      const outputs = (n.frontmatter.outputs as string[]) ?? [];
      const outStr = outputs.length > 0
...
export async function resolveIntent(
  ...
  if (!match) {
    ...
    throw new Error(
      `Intent resolved to "${target}" but no note with that name exists. Available: ${
