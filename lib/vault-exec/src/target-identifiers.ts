...
export function buildTargetIdentifierIndex(targetNames: string[]): TargetIdentifierIndex {
  const sortedNames = [...targetNames].sort((a, b) => {
    const byNorm = normalizeTargetIdentifier(a).localeCompare(normalizeTargetIdentifier(b));
 ...
 */
export function resolveTargetIdentifier(reference: string, index: TargetIdentifierIndex): TargetIdentifier | undefined {
  return index.byName.get(reference) ?? index.byId.get(reference) ?? index.byAlias.get(reference);
}
