...
- `setEdges(source, targets)` persists canonical targets: deduplicated and code-point sorted.
- `getAllTraces()` returns KV-key order: timestamp bucket, then process-local sequence, then uuid.
- `listVirtualEdges()` sorts by score descending, then source ascending, then target ascending.
...
- `insertTrace()` preserves caller-provided `executedAt`, otherwise storage assigns current UTC ISO timestamp.
