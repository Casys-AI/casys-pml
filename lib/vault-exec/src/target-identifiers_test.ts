...
Deno.test("normalizeTargetIdentifier handles accents and spaces", () => {
  assertEquals(normalizeTargetIdentifier("Équipe Vente Nord"), "equipe-vente-nord");
  assertEquals(normalizeTargetIdentifier("  Bench   Tier__Target  "), "bench-tier-target");
});
...
Deno.test("buildTargetIdentifierIndex resolves by exact name, id and shorthand", () => {
  const index = buildTargetIdentifierIndex(["Bench Tier Target", "CRM Pipeline"]);
  const bench = index.entries.find((entry) => entry.name === "Bench Tier Target")!;

  assertEquals(bench.id, "bench-tier-target");
  assertEquals(bench.alias, "b-t-t");
  assertEquals(resolveTargetIdentifier("Bench Tier Target", index)?.name, "Bench Tier Target");
  assertEquals(resolveTargetIdentifier("bench-tier-target", index)?.name, "Bench Tier Target");
  assertEquals(resolveTargetIdentifier("b-t-t", index)?.name, "Bench Tier Target");
});
...
Deno.test("collision handling for ids and aliases is deterministic", () => {
  ...
  const mapA = new Map(indexA.entries.map((entry) => [entry.name, { id: entry.id, alias: entry.alias }]));
  const mapB = new Map(indexB.entries.map((entry) => [entry.name, { id: entry.id, alias: entry.alias }]));
  ...
  assertEquals(mapA.get("Customer Data"), { id: "customer-data", alias: "c-d-4" });
