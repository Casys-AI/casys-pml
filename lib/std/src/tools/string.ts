...
export const stringTools: MiniTool[] = [
  ...
  },
  {
    name: "regex_test",
    description:
      "Test a regular expression pattern against text. Returns all matches with capture groups, positions, and highlights. Supports flags (g, i, m, s, u). Useful for regex debugging, pattern validation, or text extraction. Keywords: regex, regular expression, pattern match, regex test, capture groups, regex debug.",
    category: "string",
    inputSchema: {
      type: "object",
      properties: {
        pattern: { type: "string", description: "Regular expression pattern (without delimiters)" },
        flags: { type: "string", description: "Regex flags: g (global), i (case-insensitive), m (multiline), s (dotAll), u (unicode). Default: 'g'" },
        testString: { type: "string", description: "Text to test the pattern against" },
      },
    ...
    handler: ({ pattern, flags = "g", testString }) => {
      try {
        ...
        let match;
        const text = testString as string;
