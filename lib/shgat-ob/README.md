...
Pure TypeScript + OpenBLAS FFI implementation of SuperHyperGraph Attention Networks.
Manual backward pass, production-proven (~80% Hit@1).

This is the **production** SHGAT implementation with BLAS-accelerated linear algebra.
For the TensorFlow.js-based version, see `lib/shgat-tf/`.
...
- **OpenBLAS FFI**: Hardware-accelerated matrix operations via Deno FFI
- **Zero npm dependencies**: Pure TypeScript + system BLAS

## Installation

```bash
# Deno
deno add @pml/shgat-ob
```

## Quick Start

```typescript
import { createSHGATFromCapabilities, type TrainingExample } from "@pml/shgat-ob";
...
```typescript
import { SHGAT, DEFAULT_SHGAT_CONFIG } from "@pml/shgat-ob";
...
```typescript
import { PERBuffer } from "@pml/shgat-ob";
