...
// Test d'intégration conditionnel: nécessite un GITHUB_TOKEN valide
const hasToken = typeof process.env.GITHUB_TOKEN === 'string' && process.env.GITHUB_TOKEN.trim() !== '';
...
(hasToken ? describe : describe.skip)('GithubArticleStructureRepository - intégration', () => {
  it(
    'compte le nombre d’articles visibles dans le repo kellyassist (md/mdx) sous content_path',
    async () => {
      ...
      // Assertion minimum: au moins 1 article
      ...
      console.log('[Integration] Articles count:', articles.length, 'sample:', sample);
    },
    30000
  );
});
