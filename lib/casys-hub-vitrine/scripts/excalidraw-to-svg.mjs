...
// ─── Label helper (MCP Excalidraw format: label prop on shapes) ──────────────

function renderLabel(el) {
  if (!el.label?.text) return "";
  const lbl = el.label;
  const sz = lbl.fontSize ?? 16;
  const lines = lbl.text.split("\n");
  const lh = r(sz * 1.3);
  const cx = r(el.x + el.width / 2);
  // Vertically center: offset up by half the total text block height
  const totalH = lines.length * sz * 1.3;
  const cy = r(el.y + el.height / 2 - totalH / 2 + sz);
  const fill = lbl.strokeColor ?? el.strokeColor ?? "#e5e5e5";

  if (lines.length === 1) {
    return `\n  <text x="${cx}" y="${cy}" font-family='${FONT}' font-size="${sz}" fill="${fill}" text-anchor="middle">${esc(lines[0])}</text>`;
  }
  const spans = lines
    .map((line, i) => `    <tspan x="${cx}"${i > 0 ? ` dy="${lh}"` : ""}>${esc(line)}</tspan>`)
    .join("\n");
  return `\n  <text font-family='${FONT}' font-size="${sz}" fill="${fill}" text-anchor="middle" y="${cy}">\n${spans}\n  </text>`;
}
...
function renderRect(el) {
  ...
  let out = `  <rect x="${r(el.x)}" y="${r(el.y)}" width="${r(el.width)}" height="${r(el.height)}"${rx ? ` rx="${r(rx)}"` : ""} fill="${fillOf(el)}" stroke="${el.strokeColor}" stroke-width="${sw}"/>`;
  out += renderLabel(el);
  return out;
}
...
function renderDiamond(el) {
  ...
  const sw = el.strokeWidth ?? 1;
  let out = `  <path d="M${cx} ${cy - hh}l${hw} ${hh} ${-hw} ${hh} ${-hw} ${-hh}Z" fill="${fillOf(el)}" stroke="${el.strokeColor}" stroke-width="${sw}" stroke-linejoin="round"/>`;
  out += renderLabel(el);
  return out;
}
...
function renderEllipse(el) {
  const sw = el.strokeWidth ?? 1;
  let out = `  <ellipse cx="${r(el.x + el.width / 2)}" cy="${r(el.y + el.height / 2)}" rx="${r(el.width / 2)}" ry="${r(el.height / 2)}" fill="${fillOf(el)}" stroke="${el.strokeColor}" stroke-width="${sw}"/>`;
  out += renderLabel(el);
  return out;
}
...
function renderText(el) {
  ...
  if (lines.length === 1) {
    return `  <text x="${tx}" y="${ty}" font-family='${FONT}' font-size="${sz}" fill="${el.strokeColor}" text-anchor="${anchor}">${esc(lines[0])}</text>`;
  }

  const spans = lines
    .map((line, i) => `    <tspan x="${tx}"${i > 0 ? ` dy="${lh}"` : ""}>${esc(line)}</tspan>`)
    .join("\n");
  return `  <text font-family='${FONT}' font-size="${sz}" fill="${el.strokeColor}" text-anchor="${anchor}" y="${ty}">\n${spans}\n  </text>`;
}
...
function convertFile(inputPath) {
  ...
  // Filter out MCP-specific elements: cameraUpdate, and oversized bg rectangles
  elements = elements.filter((el) => {
    if (el.type === "cameraUpdate") return false;
    // Skip giant background rects (MCP Excalidraw pattern: huge rect as canvas bg)
    if (el.type === "rectangle" && (el.width ?? 0) > 5000 && (el.height ?? 0) > 5000) return false;
    return true;
  });
