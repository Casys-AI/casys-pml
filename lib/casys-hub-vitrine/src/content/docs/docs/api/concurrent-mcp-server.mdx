...
<Aside type="note">
  Works on **Deno** and **Node.js** (18+). The framework uses a runtime adapter (`RuntimePort`) so the same API works on both platforms. `maxBodyBytes` enforcement is implemented at both the Deno and Node.js HTTP layers.
</Aside>
...
interface ConcurrentServerOptions {
  ...
  resourceCsp?: CspOptions;              // CSP meta-tag injection for MCP Apps HTML
}
```
...
Returns an `HttpServerInstance` with `shutdown()` and `addr`:
...
<Aside type="tip">
  Security-first defaults: `maxBodyBytes` limits payload to 1 MB (returns 413), `ipRateLimit` returns 429 with `Retry-After` header, `corsOrigins` logs a warning when set to `"*"`, and `requireAuth: true` fails fast at startup if no auth is configured.
</Aside>
