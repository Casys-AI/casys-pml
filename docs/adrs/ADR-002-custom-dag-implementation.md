...
Implement DAG builder and executor from scratch, no external graph libraries for core DAG operations.

## Context

Story 2.1 AC explicitly requires "custom, zero external dependency" for DAG implementation. The system needs topological sorting and parallel execution capabilities.
...
This ADR applies to core DAG operations. Graphology (ADR-005) is used separately for GraphRAG algorithms (PageRank, community detection) which are non-trivial to implement correctly.
