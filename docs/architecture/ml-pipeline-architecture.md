...
> **Recent Changes (lib/shgat extraction):**
> - SHGAT extracted to standalone library: `lib/shgat/`
> - 16 K-heads with adaptive sizing based on graph size
> - BLAS optimization for backward pass (FFI bindings)
> - PER training with curriculum learning
> - V→V co-occurrence message passing from n8n workflow patterns
> - ADR-053: Adaptive K-head, ADR-054: IDecisionLogger, ADR-055: preserveDim mode

## Overview

Architecture complète pour un système de capability retrieval avec:
- **SHGAT** (Super Hypergraph Attention) pour le scoring - **Now in lib/shgat/**
- **PER** (Prioritized Experience Replay) pour l'apprentissage - **BLAS optimized**
- **TD Learning** (Temporal Difference) pour les value estimates
- **Hybrid Embeddings** (BGE + Node2Vec+) pour les représentations - **Edge weight-aware**
...
> **lib/shgat Structure (Jan 2026):**
> ```
> lib/shgat/
> ├── mod.ts                    # Entry point
> └── src/
>     ├── attention/            # K-head attention scorers
>     │   ├── khead-scorer.ts   # 16-head adaptive attention
>     │   ├── multi-level-scorer.ts
>     │   └── v1-scorer.ts
>     ├── core/                 # Main SHGAT implementation
>     │   ├── shgat.ts          # Core class
>     │   ├── factory.ts        # Algorithm factory
>     │   └── types.ts          # Type definitions
>     ├── message-passing/      # Graph message passing
>     │   ├── v2v.ts            # V→V co-occurrence (n8n patterns)
>     │   ├── v2e.ts            # V→E propagation
>     │   └── e2v.ts            # E→V aggregation
>     ├── training/             # Training infrastructure
>     │   ├── per-buffer.ts     # Prioritized Experience Replay
>     │   ├── blas-backward.ts  # BLAS-optimized backward pass
>     │   └── curriculum.ts     # Curriculum learning
>     └── utils/
>         └── blas-ffi.ts       # BLAS FFI bindings
> ```

```typescript
interface SHGATTrainer {
  // Model (now from lib/shgat)
  shgat: SHGAT;
