...
> **Recent Changes (Migration 039):**
> - Multi-tenant user context: `user_id UUID` added to capability tables
> - FQDN format updated: `{user}.{org}.{project}.{namespace}.{action}.{hash}`
> - User scope helper: `src/lib/user.ts` for FQDN generation
...
**FQDN Format (Updated Migration 039):** `{user}.{org}.{project}.{namespace}.{action}.{hash}`

Example: `u-abc123.local.default.fs.read_json.a7f3`

| Segment | Description | Example |
|---------|-------------|---------|
| `user` | User scope prefix (UUID prefix) | `u-abc123` |
| `org` | Organization | `local` |
| `project` | Project name | `default` |
| `namespace` | Tool namespace | `fs` |
| `action` | Action name | `read_json` |
| `hash` | Content hash | `a7f3` |
