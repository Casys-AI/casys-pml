...
> **Recent Changes:**
> - Gateway: RequestQueue concurrency control, periodic metrics logging
> - Monitoring: OTEL native support, IDecisionLogger abstraction
> - Logging: Promtail → Alloy migration
> - PML Package: Hybrid routing (client/server), session management
...
### Grafana Stack (Updated - Alloy replaces Promtail)
...
> **Migration Note (Jan 2026):** Promtail → Alloy for unified log/metric collection

### IDecisionLogger Abstraction (ADR-054)

```typescript
interface IDecisionLogger {
  logDecision(algorithm: string, decision: Decision): void;
  logFeatureContribution(feature: string, weight: number): void;
  logThresholdUpdate(tool: string, before: number, after: number): void;
}
```

Standardized algorithm telemetry across SHGAT, DR-DSP, Thompson, and Unified Search.
