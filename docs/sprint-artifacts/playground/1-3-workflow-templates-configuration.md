...
- Configuration uses Codespace workspace path: `/workspaces/Casys PML`
...
- `src/cli/utils.ts` - Added env var support to `getCasys PMLDatabasePath()`
...
- `src/cli/utils.ts` - Added `CAI_DB_PATH` env var support to `getCasys PMLDatabasePath()`
- `src/db/client.ts` - Modified `createDefaultClient()` to use `getCasys PMLDatabasePath()` instead
  of hardcoded path
- `tests/unit/cli/utils_test.ts` - Added 2 tests for custom DB path functionality
- `.devcontainer/playground/devcontainer.json` - Added `remoteEnv` with `CAI_DB_PATH` for
...
- Note: Story 1.5 pourra automatiser la configuration de `CAI_DB_PATH` pour les notebooks
