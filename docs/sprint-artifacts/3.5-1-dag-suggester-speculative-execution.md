...
As an AI agent, I want Casys PML to predict and execute likely next actions speculatively, so that
...
11. **User-defined workflows** via simple `config/workflow-templates.yaml` (Story 5-2):
    ...
    - `cai workflows sync` imports YAML → DB (source: 'user')
    - `cai workflows export` exports learned patterns → YAML for review
...
13. **Learned patterns → workflow suggestions** (synergie Story 5-2):
    ...
    - `cai workflows suggest` shows candidates
...
- [ ] Task 6: Implement agent hints + learned pattern export (AC: #12, #13)
  ...
  - [ ] 6.4: Create `cai workflows suggest` → shows learned patterns as YAML
  - [ ] 6.5: Create `cai workflows export` → DB → workflow-templates.yaml (for user review)
...
2. `cai workflows sync` → imports to DB (source: 'user')
...
│         │                                                        │
│         ▼                                                        │
│  2. `cai workflows sync` → DB (source: 'user')            │
│         │                                                        │
│         ▼                                                        │
...
│         │                                                        │
│         ▼                                                        │
│  5. `cai workflows suggest` → shows candidates            │
│         │                                                        │
│         ▼                                                        │
...
- `cai workflows suggest/export` CLI commands (AC #13 partial)
