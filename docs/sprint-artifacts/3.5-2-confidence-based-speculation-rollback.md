...
4. CLI commands:
   ...
   - `cai speculation config --enable/--disable` - Toggle
   - `cai speculation config --timeout 15000` - Set timeout
   - `cai speculation stats` - Show metrics (hit_rate, net_benefit_ms)
...
- Note: Considérer ajouter `cai speculation config --validate` pour dry-run validation sans
