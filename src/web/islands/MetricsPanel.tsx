...
export default function MetricsPanel({ apiBase: apiBaseProp }: MetricsPanelProps) {
  ...
  useEffect(() => {
    ...
    const baseOptions = {
      ...
      scales: {
        x: {
          ...
          ticks: { color: "#8a8078", maxRotation: 45, font: { size: 11 } },
        },
        y: {
          grid: { color: "rgba(255,184,111,0.08)" },
          ticks: { color: "#8a8078", font: { size: 11 } },
          beginAtZero: true,
        },
      },
    };
    ...
    // Time series charts
    const timeseriesConfigs: Array<
      {
        key: "edges" | "confidence" | "workflows";
        data: any[];
        color: string;
        type: string;
        label: string;
    ...
    > = [
      {
        key: "edges",
        data: metrics.timeseries.edgeCount,
        color: "#ffb86f",
        type: "line",
        label: "Edge Count",
      },
      {
        key: "confidence",
        data: metrics.timeseries.avgConfidence,
        color: "#4ade80",
        type: "line",
      ...
      {
        key: "workflows",
        data: metrics.timeseries.workflowRate,
        color: "#60a5fa",
        type: "bar",
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: {
                label: (ctx: any) =>
                  ` ${ctx.label}: ${ctx.raw} (${
                    ((ctx.raw / (accepted + filtered + rejected)) * 100).toFixed(1)
                  }%)`,
              },
            },
          },
      ...
      chartInstances.current.scores = new Chart(chartRefs.scores.current.getContext("2d"), {
        ...
        options: {
          ...
          scales: {
            ...
            y: {
              grid: { display: false },
              ticks: { color: "#c9c2b8", font: { size: 12, weight: "bold" } },
            },
          },
        },
      });
  ...
  useEffect(() => {
    ...
    chartInstances.current.scoreDistribution = new Chart(
      chartRefs.scoreDistribution.current.getContext("2d"),
      {
        type: "bar",
        data: {
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: { color: "#8a8078", maxRotation: 45, font: { size: 9 } },
          },
          y: {
            grid: { color: "rgba(255,184,111,0.1)" },
            ticks: { color: "#8a8078", font: { size: 9 } },
            beginAtZero: true,
          },
        },
      },
  ...
  const getSuccessColor = (rate: number) =>
    rate >= 90 ? "var(--success)" : rate >= 70 ? "var(--warning)" : "var(--error)";
          ...
          {metrics && !loading && (
            <>
              ...
              <div class="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-8 gap-3 mb-6">
                ...
                <MetricCard
                  label="Density"
                  value={`${(metrics.current.density * 100).toFixed(1)}%`}
                />
                <MetricCard label="Communities" value={metrics.current.communitiesCount} />
                <MetricCard
                  label="Capabilities"
                  value={metrics.current.capabilitiesCount || 0}
                  color="var(--info)"
                />
              ...
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                ...
                >
                  ...
                  <div class="grid grid-cols-2 gap-3 mb-4">
                    <MetricCard
                      label="New Edges"
                      value={`+${metrics.period.newEdgesCreated}`}
                      color="var(--success)"
                    />
                    <MetricCard
                      label="New Nodes"
                      value={`+${metrics.period.newNodesAdded}`}
                      color="var(--success)"
                    />
                    <MetricCard
                      label="Dependencies"
                      value={metrics.current.dependenciesCount || 0}
                    />
                    <MetricCard
                      label="Adaptive α"
                      value={metrics.current.adaptiveAlpha.toFixed(3)}
                      color="var(--accent)"
                    />
                  </div>
                  <ProgressBar
                    value={metrics.current.adaptiveAlpha}
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  ...
                  >
                    <div class="flex justify-between items-center mb-4">
                      ...
                      <span
                        class="text-xs px-2 py-1 rounded"
                        style={{
                          background: getScoreColor(metrics.algorithm.avgFinalScore).replace(
                            "var(--",
                            "rgba(",
                          ).replace(")", ", 0.15)"),
                  ...
                  >
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      ...
                      <MetricCard
                        label="Final Score"
                        value={metrics.algorithm.avgFinalScore.toFixed(3)}
                        color={getScoreColor(metrics.algorithm.avgFinalScore)}
                      />
                      <MetricCard
                        label="Semantic"
                        value={metrics.algorithm.avgSemanticScore.toFixed(3)}
                        color="#a78bfa"
                      />
                      <MetricCard
                        label="Graph"
                        value={metrics.algorithm.avgGraphScore.toFixed(3)}
                        color="#34d399"
                      />
                    </div>
                    <div class="mt-4 pt-4" style={{ borderTop: "1px solid var(--border)" }}>
                        ...
                        <>
                          <div class="grid grid-cols-3 gap-2 mb-3">
                            ...
                            <MetricCard
                              ...
                              color={getSuccessColor(stats.acceptanceRate * 100)}
                              compact
                            />
                          </div>
                      ...
                      {metrics.algorithm!.thresholdEfficiency && (
                        <div class="flex-1 min-w-[150px]">
                          <ProgressBar
                            ...
                            label={`Reject: ${
                              metrics.algorithm!.thresholdEfficiency.rejectedByThreshold
                            }/${metrics.algorithm!.thresholdEfficiency.totalEvaluated}`}
                            color="var(--warning)"
                            height={6}
                          />
                        </div>
    ...
    >
      ...
      {metrics && !loading && (
        <>
          ...
          <SectionCard title="Learning" badge={metrics.current.capabilitiesCount || 0}>
            <div class="grid gap-1.5 grid-cols-2">
              {metrics.current.capabilitiesCount !== undefined && (
                <MetricCard
                  label="Capabilities"
                  value={metrics.current.capabilitiesCount}
                  color="var(--info)"
                  compact
                />
              )}
          ...
          <SectionCard
            title="Top PageRank"
            badge={metrics.current.pagerankTop10?.length || 0}
            defaultOpen={false}
          >
