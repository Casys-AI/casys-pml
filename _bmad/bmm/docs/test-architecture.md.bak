...
**To disable**: Set `tea_use_mcp_enhancements: false` in `.bmad/core/config.yaml` OR remove MCPs from IDE config.
...
**Enable during BMAD installation** by answering "Yes" when prompted, or manually set `tea_use_playwright_utils: true` in `.bmad/core/config.yaml`.

**To disable**: Set `tea_use_playwright_utils: false` in `.bmad/core/config.yaml`.
