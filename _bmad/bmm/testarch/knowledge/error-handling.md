...
Treat expected failures explicitly: intercept network errors, assert UI fallbacks (error messages
visible, retries triggered), and use scoped exception handling to ignore known errors while catching
regressions. Test retry/backoff logic by forcing sequential failures (500 → timeout → success) and
validate telemetry logging. Log captured errors with context (request payload, user/session) but
redact secrets to keep artifacts safe for sharing.

## Rationale

Tests fail for two reasons: genuine bugs or poor error handling in the test itself. Without explicit
error handling patterns, tests become noisy (uncaught exceptions cause false failures) or silent
(swallowing all errors hides real bugs). Scoped exception handling
(Cypress.on('uncaught:exception'), page.on('pageerror')) allows tests to ignore documented, expected
errors while surfacing unexpected ones. Resilience testing (retry logic, graceful degradation)
...
test.describe("Error Telemetry", () => {
  ...
  test("should send errors to Sentry with breadcrumbs", async ({ page }) => {
    ...
    await page.route(
      "**/api/users",
      (route) => route.fulfill({ status: 403, body: { error: "Forbidden" } }),
    );
...
test.describe("Service Unavailability", () => {
  ...
  test("should show fallback UI when analytics service fails", async ({ page }) => {
    ...
    await page.route(
      "**/analytics/track",
      (route) => route.fulfill({ status: 503, body: "Service unavailable" }),
    );
...
- Used in workflows: `*automate` (error handling test generation), `*test-review` (error pattern
  detection)
