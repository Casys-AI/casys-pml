...
export function calculatePriority(factors: PriorityFactors): Priority {
  ...
  if (
    revenueImpact === "critical" || securityRisk || complianceRequired ||
...
- **Used in workflows**: `*automate` (priority-based test generation), `*test-design` (scenario
  prioritization), `*trace` (coverage validation by priority)
- **Related fragments**: `risk-governance.md` (risk scoring), `probability-impact.md` (impact
  assessment), `selective-testing.md` (tag-based execution)
